...
class _ProductEditPageState extends State<ProductEditPage> {
  ...
  void initState() {
    ...
    if (widget.productId != null) {
      final state = context.read<FlightsBloc>().state;
      if (state is FlightsLoadSuccess) {
        try {
          editing = state.flights.firstWhere((f) => f.id == widget.productId);
        } catch (e) {
          editing = null;
        }
      }
            ...
            children: [
              ...
              ElevatedButton(
                onPressed: () {
                  ...
                  if (editing != null) {
                    context.read<FlightsBloc>().add(UpdateFlight(flight));
                  } else {
                    context.read<FlightsBloc>().add(AddFlight(flight));
                  }
